...
DIGIT g_Score_Digit = {NULL};//リザルト描画に使うやつ
float Tu = 0;//リザルト用
float Tv = 0;
...
void Draw(void)
{
	...
	if (SUCCEEDED(g_pD3DDevice->BeginScene()))
	{
		...
		case RESULT:
			//g_Score_Digit.DigitOne = UI::Score % 10;//1の位
			//g_Score_Digit.DigitTen = (UI::Score % 100) / 10;//10の位
			//g_Score_Digit.DigitHundret = (UI::Score % 1000) / 100;//100の位
			//g_Score_Digit.DigitThousand = (UI::Score % 10000) / 1000;//1000の位
			////リザルトの描画処理
